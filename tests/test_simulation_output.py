...
def test_simulation_writes_expected_files(tmp_path: Path):
    ...
    assert (out.parent / "goal_dashboard.md").exists()
    assert (out.parent / "goal_dashboard.json").exists()
    ...
    assert all("source_weight" in row for row in rows)
    assert all("account_fit_weight" in row for row in rows)
    assert all("urgency_weight" in row for row in rows)
    assert all("weighted_view_score" in row for row in rows)
    assert all("weighted_follow_score" in row for row in rows)
    assert all("weighted_share_score" in row for row in rows)
